...
import { 
  ...
  Layers, Mic2, Settings, Search, History, Clock, Loader2, Zap
} from 'lucide-react';

// --- UTILS PER IL PITCH DETECTION ---
...
const App: React.FC = () => {
  ...
  // --- REFS PER IL MOTORE AUDIO ---
  const samplerRef = useRef<Tone.Sampler | null>(null);
  const analyserRef = useRef<Tone.Analyser | null>(null);
  const recorderRef = useRef<Tone.Recorder | null>(null);

  // --- INIZIALIZZAZIONE CON EFFETTI (CORPOSITÀ) ---
  const initEngine = async () => {
    await Tone.start();
    
    // 1. EFFETTI PROFESSIONALI
    const reverb = new Tone.Reverb({ decay: 4, wet: 0.35 }).toDestination();
    const chorus = new Tone.Chorus(4, 2.5, 0.5).connect(reverb).start();
    const distortion = new Tone.Distortion(0.12).connect(chorus); // Calore analogico
    ...
    if (mode === 'MIDI' && rms > 0.025) {
      ...
      if (freq) {
        ...
        if (note !== currentNote) {
          ...
          // EPIC LAYER (Ottava Bassa)
          if (isEpicMode) {
            samplerRef.current.triggerAttack(midiToNoteName(midi - 12), undefined, 0.45);
          }
          setCurrentNote(note);
        }
      }
    ...
    <div className="fixed inset-0 bg-[#0a0a0a] text-zinc-300 font-sans p-4 flex flex-col gap-5 overflow-hidden">
      ...
      {!isStarted ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <button onClick={initEngine} className="bg-white text-black px-12 py-5 rounded-[2.5rem] font-black text-xl shadow-2xl active:scale-95 transition-all uppercase tracking-tighter">Start Engine</button>
        </div>
      ) : (
        <>
          {/* SEZIONE SCALE & NOTE */}
            ...
            <div className="bg-[#121212] p-5 rounded-[2.2rem] border border-white/5">
              ...
              <div className="flex gap-1">
                {['CHR', 'MAJ', 'MIN', 'PEN'].map(s => (
                  <button key={s} onClick={() => setSelectedScale(s)} className={`flex-1 py-2 rounded-xl text-[9px] font-black transition-all ${selectedScale === s ? 'bg-purple-600 text-white shadow-lg' : 'bg-zinc-800/40 text-zinc-600'}`}>{s}</button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-[#121212] rounded-[2.5rem] p-6 border border-white/5 flex-1 overflow-y-auto scrollbar-hide">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="aspect-[4/3] bg-[#1a1a1a] rounded-3xl border-2 border-purple-600 p-5 relative shadow-2xl">
                <div className="absolute top-3 right-3 bg-purple-600 text-[6px] font-black px-2 py-0.5 rounded-full text-white tracking-tighter uppercase">HD Samples</div>
              ...
              <div className="aspect-[4/3] bg-zinc-900/30 rounded-3xl p-5 border border-white/5 opacity-40 relative">
          ...
          <div className="bg-zinc-900/90 backdrop-blur-xl rounded-[2.2rem] p-4 mb-2 border border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-4 pl-2">
               <Activity className={mode === 'MIDI' ? 'text-purple-500 animate-pulse' : 'text-zinc-700'} size={20} />
               <div>
                  <p className="text-[7px] font-black text-zinc-600 uppercase tracking-widest">Engine Status</p>
                  <p className="text-sm font-black text-white italic uppercase tracking-tighter">{mode}</p>
               </div>
            </div>
            <div className="flex items-center gap-4">
              {/* Barra Volume Dinamica */}
              <div className="w-16 h-1 bg-zinc-800 rounded-full overflow-hidden">
                 <div className="h-full bg-purple-500 transition-all duration-75" style={{ width: `${Math.min(100, rmsVolume * 900)}%` }} />
              </div>
              {/* Tasto EPIC LAYER (Arancione quando attivo) */}
              <button 
                onClick={() => setIsEpicMode(!isEpicMode)}
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${isEpicMode ? 'bg-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.5)]' : 'bg-zinc-800'}`}
              >
